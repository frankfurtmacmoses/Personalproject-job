...
*Standardized Generic Watchmen Class*
  - **Targets:**  Each Watchman monitors at least one unique target, which has its own AWS
    Simple Notification Service (SNS) topic. Targets are usually files, endpoints, or
...
*Proprietary Watchmen Class*
  ...
  - **SummarizeResult():** This function puts all notification information inside of a
...
```
  ...
  - Notifier: *cyberint-watchmen/watchmen/common/sns_notifier.py*
    ...
    - NOTE: There is a seperate notifier in
      *cyberint-watchmen/watchmen/common/notifier.py*, but it is intended for non-AWS
      notifications. As of now, all Watchmen use AWS for their notifications, so they use the sns_notfier.
...
please replace “None” with the appropriate replacement.
...
method, which can be found in *cyberint-watchmen/watchmen/main.py.* All the Watchmen
follow a similar template, so your Watchman should look like the following:
```
...
Next, you need to make some additions to *cloudformation/cloudformation.yaml.*
...
NOTE: Steps 1-3 are required. If your Watchman requires new/special permissions such
as access to a Virtual Private Cloud (VPC) or an S3 permission that has not been used
before, make sure to add it properly to the CloudFormation. For example, the Jupiter
Watchman uses a security group; therefore, it has additional Lambda properties that
the other Watchmen do not require.

4) Add your Watchman’s Security Group
...
nfoblox-CTO/cyberint-watchmen master.
