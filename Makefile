...
############################################################
POSTMAN_API := functests/api.postman_collection.json
POSTMAN_ENV_DEV:= functests/api.postman_environment.json

newman: clean-cache
	@echo ""
	npm list -g newman || npm install -g newman
	@echo "-----------------------------------------------------------------------"
	@echo "Running functional tests [dev] ... "
	@echo ""
	newman run $(POSTMAN_API) -e $(POSTMAN_ENV_DEV) --insecure
	@echo ""
	@echo "- DONE: $@"
...
ifeq ("$(DOCKER_DENV)","")
	...
	@echo "......................................................................."
	@echo ""
	@echo "Starting watchmen [in detached mode] ..."
	AWS_ACCESS_KEY_ID=$(AWS_ACCESS_KEY_ID) \
	AWS_SECRET_ACCESS_KEY=$(AWS_SECRET_ACCESS_KEY) \
	AWS_DEFAULT_REGION=$(AWS_DEFAULT_REGION) \
	DOCKER_PORT=$(DOCKER_PORT) \
	API_PORT=$(DOCKER_PORT) \
	docker-compose -f docker-compose.yml -p $(DOCKER_APIS) up -d --remove-orphans
else
	@echo "Cannot start docker-compose."
endif
	@echo ""
	@echo "- DONE: $@ [detached]"
...
run-stop stop:
	@echo ""
ifeq ("$(DOCKER_DENV)","")
	@echo "Shutting down watchmen ..."
	docker-compose -f docker-compose.yml -p $(DOCKER_APIS) down
	@for srv in $(DOCKER_APIS); do \
	echo - stopping $$srv $$(docker stop $$srv 2>/dev/null) ...; \
	echo - removing $$srv $$(docker rm -f $$srv 2>/dev/null) ...; \
	done
else
	@echo "Cannot run docker."
endif
	@echo ""
	@echo "- DONE: $@"
...
SWAGGER_PATH := $(PWD)/$(PROJECT)/apidoc/v1
